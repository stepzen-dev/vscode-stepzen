...
export async function resolveStepZenProjectRoot(
  ...
  if (start) {
    ...
    if (!start.fsPath || typeof start.fsPath !== 'string') {
      ...
      if (byAscend) return byAscend;
    }
  }

  // ② otherwise scan the entire workspace ---------------------------------
  stepzenOutput.appendLine(`StepZen project not found in current folder. Scanning...`);
  const configs = await vscode.workspace.findFiles("**/stepzen.config.json", "**/node_modules/**");
  ...
  stepzenOutput.appendLine(`Multiple StepZen projects found. Prompting for selection...`);
  
  // Validate that all configs have valid paths
  const validConfigs = configs.filter(c => c.fsPath && typeof c.fsPath === 'string');
  ...
  if (!pick || !pick.target) throw createError(
  ...
  function ascendForConfig(dir: string): string | null {
    ...
    if (!dir || typeof dir !== 'string') {
      stepzenOutput.appendLine('Error: Invalid directory path provided to ascendForConfig');
      return null;
    }
