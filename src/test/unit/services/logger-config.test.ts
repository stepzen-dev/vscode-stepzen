...
suite("Logger Configuration Test Suite", () => {
  ...
  test("should respect logLevel configuration from settings", () => {
    ...
    const errorLogs = mockOutputChannel.__messages.filter((msg) =>
      msg.includes("[ERROR]"),
    );
    const warnLogs = mockOutputChannel.__messages.filter((msg) =>
      msg.includes("[WARN]"),
    );
    const infoLogs = mockOutputChannel.__messages.filter((msg) =>
      msg.includes("[INFO]"),
    );
    const debugLogs = mockOutputChannel.__messages.filter((msg) =>
      msg.includes("[DEBUG]"),
    );
    ...
    assert.strictEqual(
      warnLogs.length,
      0,
      "Warning messages should not be logged",
    );
    assert.strictEqual(
      infoLogs.length,
      0,
    ...
    assert.strictEqual(
      ...
      5,
      "All messages should be logged with debug level",
    );
  });
  ...
  test("logger level enum values should be correctly ordered", () => {
    ...
    assert.notStrictEqual(
      LogLevel.ERROR,
      LogLevel.WARN,
      "ERROR should be different from WARN",
    );
    assert.notStrictEqual(
      LogLevel.WARN,
      LogLevel.INFO,
      "WARN should be different from INFO",
    );
    assert.notStrictEqual(
      LogLevel.INFO,
      LogLevel.DEBUG,
      "INFO should be different from DEBUG",
    );
  });
  ...
  test("setLogLevel should change logging behavior", () => {
    ...
    assert.ok(
      mockOutputChannel.__messages.some((msg) => msg.includes("[ERROR]")),
      "Message should include ERROR level",
    );
