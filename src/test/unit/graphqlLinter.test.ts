...
suite("GraphQL Linter Test Suite", () => {
  ...
  test("should detect anonymous operations", async () => {
    ...
    const anonymousRule = rules.find((r: any) => r.name === 'no-anonymous-operations');
    
    assert.ok(anonymousRule, "Anonymous operations rule should exist");
    
    const issues = anonymousRule.check(ast);
    assert.strictEqual(issues.length, 1, "Should detect one anonymous operation");
    assert.strictEqual(issues[0].message, 'Anonymous operations are not allowed. Please provide a name for this operation.');
  ...
  test("should allow named operations", async () => {
    ...
    const issues = anonymousRule.check(ast);
    assert.strictEqual(issues.length, 0, "Should not detect issues with named operations");
  });
  ...
  test("should detect duplicate fields", async () => {
    ...
    const ast = parse(content);
    const rules = (linter as any).rules;
    const duplicateRule = rules.find((r: any) => r.name === 'no-duplicate-fields');
    ...
    assert.ok(issues.length >= 1, `Should detect at least one duplicate field, got ${issues.length}`);
    assert.ok(issues.some((d: any) => d.message.includes("Duplicate field 'name' found in type 'User'")));
    assert.strictEqual(issues[0].severity, 'error');
  });
  ...
  test("should detect missing descriptions", async () => {
    ...
    const ast = parse(content);
    const rules = (linter as any).rules;
    const descriptionRule = rules.find((r: any) => r.name === 'require-description');
    ...
    assert.ok(issues.length >= 2, `Should detect at least two missing descriptions, got ${issues.length}`);
    assert.ok(issues.some((d: any) => d.message.includes("Type 'User' should have a description")));
    assert.ok(issues.some((d: any) => d.message.includes("Field 'id' in type 'User' should have a description")));
  ...
  test("should allow descriptions", async () => {
    ...
    const issues = descriptionRule.check(ast);
    assert.strictEqual(issues.length, 0, "Should not detect issues with proper descriptions");
  });
  ...
  test("should detect deprecated fields without reason", async () => {
    ...
    const ast = parse(content);
    const rules = (linter as any).rules;
    const deprecatedRule = rules.find((r: any) => r.name === 'require-deprecation-reason');
    
    assert.ok(deprecatedRule, "Require deprecation reason rule should exist");
    
    const issues = deprecatedRule.check(ast);
    assert.strictEqual(issues.length, 1, "Should detect deprecated field without reason");
    assert.strictEqual(issues[0].message, 'Deprecated fields should include a reason');
    assert.strictEqual(issues[0].severity, 'warn');
  });
  ...
  test("should allow deprecated fields with reason", async () => {
    ...
    const issues = deprecatedRule.check(ast);
    assert.strictEqual(issues.length, 0, "Should not detect issues with deprecated fields that have reasons");
  });
  ...
  test("should detect non-camelCase field names", async () => {
    ...
    const ast = parse(content);
    const rules = (linter as any).rules;
    const namingRule = rules.find((r: any) => r.name === 'field-naming-convention');
    
    assert.ok(namingRule, "Field naming convention rule should exist");
    
    const issues = namingRule.check(ast);
    assert.strictEqual(issues.length, 2, "Should detect non-camelCase field names");
    assert.ok(issues.some((d: any) => d.message.includes("Field 'first_name' should use camelCase naming convention")));
    assert.ok(issues.some((d: any) => d.message.includes("Field 'LastName' should use camelCase naming convention")));
    assert.strictEqual(issues[0].severity, 'warn');
  });
  ...
  test("should allow camelCase field names", async () => {
    ...
    const issues = namingRule.check(ast);
    assert.strictEqual(issues.length, 0, "Should not detect issues with camelCase field names");
  });
  ...
  test("should ignore special fields like __typename", async () => {
    ...
    const issues = namingRule.check(ast);
    assert.strictEqual(issues.length, 0, "Should not detect issues with special fields like __typename");
  });
  ...
  test("should detect non-nullable fields in root types", async () => {
    ...
    const ast = parse(content);
    const rules = (linter as any).rules;
    const nullableRule = rules.find((r: any) => r.name === 'root-fields-nullable');
    
    assert.ok(nullableRule, "Root fields nullable rule should exist");
    
    const issues = nullableRule.check(ast);
    assert.strictEqual(issues.length, 2, "Should detect non-nullable fields in root types");
    assert.ok(issues.some((d: any) => d.message.includes("Field 'user' in root type 'Query' should be nullable")));
    assert.ok(issues.some((d: any) => d.message.includes("Field 'users' in root type 'Query' should be nullable")));
    assert.strictEqual(issues[0].severity, 'warn');
  });
  ...
  test("should allow nullable fields in root types", async () => {
    ...
    const issues = nullableRule.check(ast);
    assert.strictEqual(issues.length, 0, "Should not detect issues with nullable fields in root types");
  });
  ...
  test("should not check non-root types for nullability", async () => {
    ...
    const issues = nullableRule.check(ast);
    assert.strictEqual(issues.length, 0, "Should not check nullability for non-root types");
  });
  ...
  test("should handle non-existent files gracefully", async () => {
    ...
    assert.strictEqual(diagnostics.length, 0, "Should return empty array for non-existent files");
  });
  ...
  test("should respect configuration changes", async () => {
    ...
    try {
      ...
      assert.strictEqual(anonymousRule, undefined, "Anonymous operations rule should not exist when disabled");
    } finally {
  ...
  test("should handle all rules disabled", async () => {
    ...
    try {
      ...
      assert.strictEqual(rules.length, 0, "Should not have any rules when all are disabled");
    } finally {
      vscode.workspace.getConfiguration = originalGetConfiguration;
    }
  });
